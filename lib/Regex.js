 ...
 * @updated Sat, Oct 20 2012 - 15:14:56 -0700
 */
...
Regex.prototype.generate = function generate ( match, mode ) {
  ...
  var mode = mode || this.mode;

  for ( var i = this.groups.length - 1; i >= 0; i -= 1 ) {
    var current = new Match('', 0, match.groups);
    var result = this.groups[i].generate(current, mode);
